...
if __name__ == "__main__":
    ...
    parser.add_argument("--view1", type=str, default="./DukeCFLocations.csv", help = "path to centerfield view")
    parser.add_argument("--view2", type=str, default="./Duke1BLocations.csv", help="path to side view")
    ...
    side = pd.read_csv(opt.view2, header=None)
    center = pd.read_csv(opt.view1, header=None)
    
    side.columns = ['Frame', 'label', 'cx', 'cy', 'w', 'h']
    center.columns = ['Frame', 'label', 'cx', 'cy', 'w', 'h']
    ...
    for frame in df.Frame:
        xval = df.cx_y[df.Frame == frame]
        yval = df.cy_y[df.Frame == frame]
